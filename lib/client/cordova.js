...
class PushHandle extends EventState {
  ...
  Initialize() {
    if (!this.options) {
      this.log('Push.Error: "Push.Initialize called before Push.Configure"');
      throw new Error('Push.Initialize called before Push.Configure');
    }
    else {
      if (!this.initialized) {
        this.log('Push.Initialize:', this.options);
        ...
        initPushUpdates(this.options.appName);
      } else {
        this.log('Push.Error: "Push.Initialize may only be called once"');
        throw new Error('Push.Initialize may only be called once');
      }
    }
  }
  emitPermission() {
    if (this.initialized) {
      PushNotification.hasPermission((function(_this) {
        return function(res) {
          _this.emit('permission', res);
        };
      })(this));
    } else {
      this.log('Push.hasPermission, Error: "Push not initialized"');
